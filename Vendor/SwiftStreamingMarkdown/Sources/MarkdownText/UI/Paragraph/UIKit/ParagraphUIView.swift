...
class ParagraphUIView: UITextView {
  ...
  func setParagraphContents(_ newContents: NSMutableAttributedString, lineSpacing: CGFloat? = nil, animatedByWord: Bool) {
    ...
    let spacingUnchanged = self.lineSpacing == lineSpacing
    if spacingUnchanged, paragraphContents.hasSameMarkdownIdentity(as: newContents) {
      return
    }

    let previous = paragraphContents
    let previousLength = previous.length
    let prefixUnchanged = spacingUnchanged && newContents.markdownPrefix(equalTo: previous)

    // Same characters and latex payloads: keep the existing attachment views.
    // A fresh parse mints new NSTextAttachment instances; replacing the storage
    // would tear down every MTMathUILabel and the formulas would flicker.
    if prefixUnchanged, newContents.length == previousLength {
    ...
    let canAppend = prefixUnchanged
      && previousLength > 0
      && finalString.length > previousLength
      && oldAttributedString.length == previousLength
    if canAppend {
